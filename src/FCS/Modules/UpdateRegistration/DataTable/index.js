...
class Index extends Component {
    ...
    async createNoRegistration() {
        ...
        const {process, studentID, organicUnit, program} = this.state;
    ...
    handleChange = field => event => {
        switch (field) {
            ...
            case 'program':
                ...
                break;
            case 'admissionPlan':
                let admissionPlanCant = $('#admissionPlan-' + event.target.value).attr('admissionplancant');
                this.setState({
                    admissionPlan: event.target.value,
                    admissionPlanCant: admissionPlanCant
                });
    ...
    render() {
        ...
        if (records.length > 0) {
            records.map((r, index) => {
                    ...
                    // const temp = r.Registration.map((m, i) => {
                    //     return (
                    //         <a href="">{m.Academic_semester.Academic_calendar.denomination.substr(-4) + ' - ' + m.Academic_semester.denomination.substr(-1) + ' ' + m.type}</a>)
                    //
                    //
                    // }).join(' | ');
                    ...
                    if (r.Registration.length > 0) {
                        ...
                        r.Registration.map((m, i) => {
                            ...
                            if (m.type === 'No Matrículado') {
                                algo =
                                    <><a key={i} style={{color: '#ff5252'}}
                                         href="#"
                                         onClick={() => this.callDestroyNoRegistration(m.id)}>{m.Academic_semester.Academic_calendar.denomination.substr(-4) + ' - ' + m.Academic_semester.denomination.substr(-1) + ' ' + m.type}</a> |
                                    </>;
                            } else {
                                algo =
            ...
            <>
                                                    ...
                                                    admissionPlans.length > 0 ?
                                                        admissionPlans.map((r, k) =>
                                                            <option id={"admissionPlan-" + r.id}
                                                                    ...
                                                                    admissionplancant={r.Cost_admission_plan && r.Cost_admission_plan.cant}
                                                                    value={r.id}
                ...
                <Modal show={this.state.modal}>
                    <Modal.Header className='bg-primary'>
                        <Modal.Title as="h5" style={{color: '#ffffff'}}>REGISTRAR NO MATRÍCULA DEL
                            ESTUDIANTE</Modal.Title>
