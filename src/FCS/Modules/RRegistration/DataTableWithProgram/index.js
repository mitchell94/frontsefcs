...
class Index extends Component {
  ...
  async createNoRegistration() {
    ...
    if (
      ...
      try {
        ...
        this.listStudentAdmissionProgramWithRegistration(
  ...
  handleChange = (field) => (event) => {
    switch (field) {
      case "program":
        ...
        break;
      case "admissionPlan":
        let admissionplanmask = $("#admissionPlan-" + event.target.value).attr(
          "admissionplanmask"
        );
        let studyPlanMask = $("#admissionPlan-" + event.target.value).attr(
          "datastudyplanmask"
        );
        this.setState({
  ...
  render() {
    ...
    const { programs, program, admissionPlans, admissionPlan, students } =
      this.state;
    const options = {
      filter: true,
      searchOpen: false,
    ...
    if (records.length > 0) {
      records.map((r, index) => {
        data.push([index + 1, r.Person.name, r.Person.document_number, r.type]);
        let temp = [];
        let algo = "";
              ...
              algo = (
                <>
                  <a
                    ...
                    style={{ color: "#ff5252" }}
                    href="#"
                    onClick={() => this.callDestroyNoRegistration(m.id)}
                  >
                    {m.Academic_semester.Academic_calendar.denomination.substr(
                      -4
                    ) +
                      " - " +
                      m.Academic_semester.denomination.substr(-2) +
                      " " +
                      m.type}
                  </a>{" "}
                  |
                        ...
                        processs.map((r, index) => {
                          return (
                            <option value={r.id} key={index}>
                              {r.Academic_calendar.denomination.substr(-4) +
                                "-" +
                                r.denomination.substr(-2)}
                            </option>
                          );
                        })
                      ) : (
              ...
              <Col xs={12} sm={12} md={12} lg={12} xl={12}>
                <Form.Group className="form-group fill">
                  <Form.Label
                    className="floating-label"
                    style={
                      this.state.type === "" ? { color: "#ff5252 " } : null
                    }
                  >
                    Tipo<small className="text-danger"> *</small>
                  </Form.Label>
                  <Form.Control
                    as="select"
