...
import {
    Row,
    Col,
    Form, OverlayTrigger, Tooltip, Button, Modal,
...
class StudyPlan extends Component {
    ...
    async createPlan() {
        ...
        const {programID, codePlan, creditRequired, descriptionPlan, mesh} = this.state;

        if (programID !== "" && codePlan !== "" && creditRequired !== "" && descriptionPlan !== "" && mesh !== "") {
    ...
    async updatePlan() {
        this.setState({studyPlanLoader: true});
        const {creditElective, codePlan, creditRequired, descriptionPlan, mesh} = this.state;

        if (codePlan !== "" && creditElective !== "" && creditRequired !== "" && descriptionPlan !== "" && mesh !== "") {
    ...
    async updateActualPlan(planID) {
        this.setState({studyPlanLoader: true});
        if (planID !== "") {
            const url = app.programs + '/' + app.plan + '/' + this.state.programID + '/actual';
            let data = new FormData();
            data.set("id_plan", planID);
            try {
                const res = await axios.patch(url, data, app.headers);
                this.setState({studyPlanLoader: false, plans: res.data.record});
                PNotify.success({title: "Finalizado", text: res.data.message, delay: 2000});
            } catch (err) {
                console.log(err)
                this.setState({studyPlanLoader: false});
    ...
    handleChange = field => event => {
        switch (field) {
            ...
            case 'descriptionPlan':
                let descriptionPlan = event.target.value.slice(0, 255).toUpperCase();
    ...
    openFormCourse = (r) => {
        this.setState({courseForm: !this.state.courseForm, plan: r.id, planMask: r.description})
    };
    closeFormCourse = () => {
        this.setState({courseForm: !this.state.courseForm, plan: "", planMask: ""})
    };
    actualPlan = (id) => {
    ...
    render() {
        ...
        const {codePlan, totalPeriod, creditElective, mesh, creditRequired} = this.state;
        return (
            <>
                                            ...
                                            <Col xs={12} sm={12} md={12} lg={12} xl={12} className='filter-bar'>
                                                ...
                                                <nav className="navbar m-b-30 p-10">
                                                    ...
                                                    <div className="nav-item nav-grid f-view">
                                                        <OverlayTrigger
                                                            overlay={<Tooltip>Cerrar</Tooltip>}>
                                                            <button style={{marginRight: "8px"}}
                                                                    onClick={() => this.closeFormCourse()} type="button"
                                                                    className="btn-icon btn btn-dark"><Close/></button>
                ...
                <Modal show={planModal} size={"xl"} backdrop="static">
                    ...
                    <Modal.Header className='bg-primary'>
                        <Modal.Title as="h5" style={{color: '#ffffff'}}>PLANES DE ESTUDIO</Modal.Title>
                        <div className="d-inline-block pull-right">

                            <OverlayTrigger
                                overlay={<Tooltip style={{zIndex: 100000000}}>Cerrar</Tooltip>}>
                                <Close style={{color: "white"}} onClick={() => this.closeFormPlan()}/>
                    ...
                    <Modal.Body>
                        ...
                        <Row>
                            ...
                            <Col xs={12} sm={12} md={6} lg={6} xl={6}>
                                <Form.Group className="form-group fill">
                                    <Form.Label className="floating-label"
                                                style={descriptionPlan === "" ? {color: "#ff5252 "} : null}>
                                        Nombre del plan
                            ...
                            <Col xs={12} sm={12} md={2} lg={2} xl={2}>
                                <Form.Group className="form-group fill">
                                    <Form.Label className="floating-label"
                                                style={codePlan === "" ? {color: "#ff5252 "} : null}>
                                        Codigo
                                        <small className="text-danger"> *</small>
                                    </Form.Label>
                            ...
                            <Col xs={12} sm={12} md={4} lg={4} xl={4}>
                                <Form.Group className="form-group fill">
                                    <Form.Label className="floating-label"
                                                style={creditRequired === "" ? {color: "#ff5252 "} : null}>
                            ...
                            <Col xs={12} sm={12} md={3} lg={3} xl={3}>
                                <Form.Group className="form-group fill">
                                    <Form.Label className="floating-label"
                                                style={mesh === "" ? {color: "#ff5252 "} : null}
                                    >Cuenta con malla ?<small className="text-danger"> *</small></Form.Label>
                                    <Form.Control as="select" value={mesh} onChange={this.handleChange('mesh')}>
                            ...
                            <Col>
                                ...
                                {actionPlan === 'add' ?
                                    <Button
                                        ...
                                        {this.state.studyPlanLoader && component.spin}
                                        Guardar</Button> :
