...
class AdministrativeForm extends React.Component {
    state = {
    ...
    createPersonAdministrative() {
        this.setState({loaderPerson: true});
        const url = app.person + '/' + app.persons + '/' + app.administrative
        const {
            studentState, teacherState, administrativeState, organicUnit, charge, contractType, date_start, date_end,
            name, paternal, maternal, documentNumber, gender, birthDate, email, civilStatu, file, cellPhone, address, birthDistrict, residentDistrict
        } = this.state;
    ...
    async createCharge() {
        this.setState({loaderPerson: true});
        const url = app.person + '/' + app.administrative;
    ...
    render() {
        ...
        const {
             address, cellPhone,  namePersonMask,
            name, paternal, maternal, documentNumber, gender, birthDate,  email, civilStatu,  civilStatus,
            charge, contractType, birthDepartment, birthProvince, birthDistrict,  residentDepartment, residentProvince, residentDistrict,

        } = this.state;

        const {charges, contractTypes, birthDepartments, birthProvinces, birthDistrics, residentDepartments, residentProvinces, residentDistrics} = this.state;
            ...
            <Aux>
                ...
                <Modal show={true} size={"xl"} backdrop="static">
                    <Modal.Header className='bg-primary'>
                        <Modal.Title as="h5" style={{color: '#ffffff'}}>REGISTRAR ADMINISTRATIVO</Modal.Title>
                        ...
                        <Row>
                                            ...
                                            <Row>
                                                <Col sm={4}>
                                                    <Form.Control as="select"
                                                        ...
                                                        >
                                                        <option defaultValue={true} hidden style={{color: "whitesmoke"}}>Departamento</option>
                                                        {
                            ...
                            <Col xs={12} sm={12} md={12} lg={12} xl={12}>
                                <Button
                                    ...
                                    variant="primary"
                                    onClick={this.state.existPerson === false ? () => this.createPersonAdministrative() : () => this.createCharge()}
                                >
