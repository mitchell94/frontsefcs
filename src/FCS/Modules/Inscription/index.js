...
// MPT
import FormChangeProgram from "./Form/change_program.js";
...
class Inscription extends Component {
    ...
    async retriveAdmissionPlanByID(id_admission_plan) {
        ...
        try {
            ...
            if (res.data !== "") {
                this.listSimpleProgramByOrganicUnitRegisterID(
                    res.data.Program.Organic_unit_register.id
                );
                this.listAdmissionPlanByProgramIDS(res.data.id_program);
                this.retriveSemesterActivity(
                    res.data.id_program,
                    res.data.id_process,
                    id_admission_plan
                );
                this.setState({
                    organicUnit: {
                        ...
                        label:
                            res.data.Program.Organic_unit_register
                                .denomination +
                            " " +
                            res.data.Program.Organic_unit_register.Campu
                                .denomination,
                    },
    ...
    handleChange = (field) => (event) => {
        switch (field) {
            case "organicUnit":
                ...
                break;
            case "program":
                let programMask = $("#programmask-" + event.target.value).attr(
                    "dataprogrammask"
                );
            ...
            case "admissionPlan":
                ...
                let admissionplanmask = $(
                    "#admissionPlan-" + event.target.value
                ).attr("admissionplanmask");
                let studyPlanMask = $(
                    "#admissionPlan-" + event.target.value
                ).attr("datastudyplanmask");
                this.setState({
                    admissionPlan: event.target.value,
                    studyPlan: studyPlan,
                    process: process,
    ...
    // MPT
    changeProgram = (student) => {
        // this.openFormChange();
        this.setState({
            stateFormChangeProgram: true,
            personId: student.Person.id,
        });
    };

    closeFormChangeProgram = () => {
        this.setState({ stateFormChangeProgram: false });
    };
    // MPT END

    render() {
        const { admissionPlan, admissionPlans, activate } = this.state;
        ...
        return (
            <>
                ...
                <div style={{ position: "relative" }}>
                    ...
                    {admissionPlan !== "" && (
                        <DataTable
                            ...
                            // MPT
                            openFormChangeProgram={this.changeProgram}
                        />
                    )}
                </div>
                ...
                <FormChangeProgram
                    // GENERAL DATA
                    isShown={this.state.stateFormChangeProgram}
                    closeForm={this.closeFormChangeProgram}
                    listPrograms={this.state.programs}
                    // FORM DATA
                    organicUnitId={this.state.organicUnit.value}
