...
import {Row, Col, Card, Table, Button, Tabs, Tab, OverlayTrigger, Tooltip, Form, Modal} from 'react-bootstrap';
...
export default class Dashboard extends Component {
    ...
    async retrivePersonProfile(id, ORGANIC_UNIT) {
        ...
        const url = app.person + '/' + app.persons + '/' + app.profile + '/' + id;
        try {
    ...
    async retrivePersonStudentProgram(id) {
        ...
        this.setState({profileLoader: true});
        const url = app.person + '/' + app.persons + '/' + app.student + '/data/' + id;
        try {
    ...
    async retrivePersonProfileGOD(id) {
        ...
        this.setState({profileLoader: true});
        const url = app.person + '/' + app.persons + '/' + app.profile + '/' + id + '/g';
        try {
            const res = await axios.get(url, app.headers);
            if (res.data) {
    ...
    setData = (r) => {
        ...
        this.setState({
            ...
            photo: r.photo ? app.server + app.personPhotography + r.photo : preview,
            address: r.address,
            ...
            birthCountryMask: r.Districts_birth.Province.Department.Country.description,
            birthDepartmentMaks: r.Districts_birth.Province.Department.description,
            ...
            resideCountryMask: r.Districts_reside.Province.Department.Country.description,
            resideDepartmentMask: r.Districts_reside.Province.Department.description,
    ...
    callData = async () => {
        if (component.ORGANIC_UNIT !== "") {
            this.retrivePersonProfile(this.state.PERSONID, component.ORGANIC_UNIT);
        } else {
    ...
    openModalObservation = (r) => {
        ...
        this.setState({modalObservation: true, observationID: r.id, observation: r.observation})
    };
    closeModalObservation = () => {

        this.setState({modalObservation: false, observationID: '', observation: ''})
    };
    ...
    updateObservationStudent() {
        ...
        const url = app.person + '/' + app.student + '/observation-update/' + this.state.observationID;
        const {
            observation,
    ...
    render() {
        ...
        let residencia = resideCountryMask + " - " + resideDepartmentMask + " - " + resideProvinceMaks + " - " + resideDistrictMaks;
        let nacimiento = birthCountryMask + " - " + birthDepartmentMaks + " - " + birthProvinceMask + " - " + birthDistrictMaks;
        const {
            documentNumber, gender, birthDate, email, photo, address, cellPhone, modalObservation, observation
        ...
        return (<Aux>
            <TitleModule
                ...
                fatherModuleUrl={"/profile"} fatherModuleTitle={"PERFIL"}
            ...
            <Row>
                <Col sm={12} className='btn-page'>
                    ...
                    <Card style={{marginBottom: "5px"}}>
                        ...
                        <Card.Body>
                            <Tabs variant="pills" defaultActiveKey="home" className="">
                                <Tab eventKey="home" title="INFORMACIÓN BASICA">
                                </Tab>
                                <Tab eventKey="profile" title="ESTUDIOS">
                                </Tab>
                                <Tab eventKey="contact" title="TRABAJO">
                                </Tab>
                            </Tabs>
                        </Card.Body>
                    </Card>
                        ...
                        <Card.Body>
                            <div className="client-detail">
                                ...
                                <div className="client-contain">
                                    ...
                                    <p className="text-muted m-0 p-t-10">{documentNumber}</p>
                                </div>
                            </div>
                                    ...
                                    <Table className="table-borderless m-b-0">
                                        ...
                                        <tr>
                                            <td className="p-0 border-0">
                                                <p className="m-b-10">Estado civil</p>
                                                <p className="m-b-10">Genero</p>
                                                <p className="m-b-10">Telefono</p>

                                                <p className="m-b-10">Lugar de Nacimimento</p>
                                                <p className="m-b-10">Fecha de Nacimimento</p>
                                                <p className="m-b-10">Lugar de residencia</p>
                                                <p className="m-b-10">Dirección</p>
                                            </td>
                                            <td className="p-0 border-0">
                                                <p className="m-b-10">Solterno</p>
                                                <p className="m-b-10">{gender}</p>
                                                <p className="m-b-10">{cellPhone}</p>

                                                <p className="m-b-10">{nacimiento}</p>
                                                <p className="m-b-10">{birthDate}</p>
                                                <p className="m-b-10">{residencia}</p>
                    ...
                    <Row>
                        ...
                        {this.state.students.length > 0 && this.state.students.map((r, i) => {
                            return (
                                            ...
                                            <Row>
                                                <Col xs={6} sm={6} md={6} lg={6} xl={6}>
                                                    <p style={{
                                                        fontSize: '15px',
                                                        fontWeight: 'bold'
                                                    }}> {r.Program.description}</p>
                                                </Col>
                                                <Col xs={6} sm={6} md={6} lg={6} xl={6}>
                                                    {(!component.ORGANIC_UNIT || component.USER_TYPE === 'Academico' || component.USER_TYPE === 'Administrador') &&
                                                        <>
                                                            ...
                                                            <OverlayTrigger
                                                                ...
                                                                <GetApp style={{color: "#fbb901", float: 'right'}}
                                                                        onClick={() => this.downloadPdf(r.id)}
                                                                />
                                                            </OverlayTrigger>
                                                    ...
                                                    <OverlayTrigger
                                                        overlay={<Tooltip>REGISTRAR OBSERVACIÓN</Tooltip>}>

                                                        <Edit style={{color: "#01afec", float: 'right'}}
                                                              onClick={() => this.openModalObservation(r)}
                                                        />
                                                    </OverlayTrigger>
                                                </Col>
                                            </Row>
                                            {
                                                (r.observation !== null && r.observation !== "null") ?
                                                    <p>{r.observation}</p>
                                                    : <p>No se ha registrado Observación.</p>
                ...
                <Modal show={modalObservation}>
                    <Modal.Header className='bg-primary'>
                        ...
                        <div className="d-inline-block pull-right">
                            <span type="button" onClick={this.closeModalObservation}> <i className="feather icon-x"
                                                                                         style={{
                                                                                             fontSize: "20px",
                                                                                             color: 'white'
                                                                                         }}></i> </span>
                        ...
                        <Row>
                            <Col xl={12} xs={12} sm={12} md={12} lg={12}>
                                ...
                                <Form.Group className="form-group fill">
                                    <Form.Label className="floating-label">Registre incidencias del estudiante<small
                ...
                {this.state.form && <ProfileForm ref={(ref) => this.Profile = ref}
