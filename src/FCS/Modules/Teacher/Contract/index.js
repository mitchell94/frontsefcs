...
class TeacherContract extends Component {
    constructor(props) {
        super(props);
        this.state = {
    ...
    async componentDidMount() {
        ...
        if (component.ORGANIC_UNIT !== "") {
            PERSONID && this.retrivePersonTeacher(PERSONID, component.ORGANIC_UNIT);
        } else {
            this.retrivePersonTeacherGOD(PERSONID);
        }
    }

    async retrivePersonTeacher(id, ORGANIC_UNIT) {
        let namePerson = "";
        this.setState({contractsLoader: true});
        const url = app.person + '/' + app.persons + '/' + app.teacher + '/' + id;
        try {
            ...
            if (res.data) {
                ...
                this.setState({namePerson, contracts: res.data.Teachers});
            }
    ...
    async retrivePersonTeacherGOD(id,) {
        let namePerson = "";
        this.setState({contractsLoader: true});
        const url = app.person + '/' + app.persons + '/' + app.teacher + '/' + id + '/g';
        try {
    ...
    callData = async () => {
        if (component.ORGANIC_UNIT !== "") {
            this.retrivePersonTeacher(this.state.PERSONID, component.ORGANIC_UNIT);
        } else {
            this.retrivePersonTeacherGOD(this.state.PERSONID);
        }
    };
        ...
        return (
            <>
                ...
                <TitleModule
                    ...
                    fatherModuleUrl={"/teacher"} fatherModuleTitle={"DOCENTES"}
...
export default withRouter(TeacherContract)