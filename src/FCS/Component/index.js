...
const pdfReportAutoPaymentProgramAdmision = async (
  ...
  for (let i = 0; i < movements.length; i++) {
    ...
    pdf.text(
      String(
        movements[i].Person.name + " / " + movements[i].Person.document_number
      ),
      53,
      30
    );
...
const pdfReportAutoRecordAcademico = async (
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    for (let j = 0; j < data[i].Registration_course.length; j++) {
      ...
      totalAprovedCredit =
        data[i].Registration_course[j].note >= 14
          ? totalAprovedCredit + data[i].Registration_course[j].credits
          : totalAprovedCredit;
...
const pdfReportAutoTableConstancyEgressSeunsm = async (data) => {
  ...
  pdf.setFont("helvetica", "normal");
  pdf.text(
    "La Coordinadora de la Unidad de Segunda Especialidad de la Facultad de Ciencias de la Salud Obsta. Mg. YNÉS TORRES FLORES de la Universidad Nacional de San Martín, hace constar que:",
    20,
    75,
    {
      maxWidth: 170,
  ...
  pdf.text(
    "Ha culminado sus estudios en el programa de " +
      ...
      ", con un total de " +
      credit +
      " créditos de acuerdo a las normas y reglamentos de la Unidad de Segunda Especialidad de la Universidad Nacional de San Martin.",
    20,
    130,
    {
      maxWidth: 170,
...
const pdfReportAutoTableConstancyRegistration = async (data) => {
  let student = data.studentData.Person.name;
  let firtsRegister = moment(data.studentData.Admission_plan.date_class).format(
    "DD-MM-YYYY "
  );
  ...
  if (registration.length > 0) {
    for (let i = 0; i < registration.length; i++) {
      ...
      for (let j = 0; j < registration[i].Registration_course.length; j++) {
        let typeCourse =
          registration[i].Registration_course[j].type_course === "Obligatorio"
            ? "O"
            : "E";
        totalCourse = totalCourse + 1;
        totalCredit =
          totalCredit + registration[i].Registration_course[j].credits;
        generalPromedi =
          generalPromedi + registration[i].Registration_course[j].note;
        totalAprovedCourse =
        ...
        aprovedPromedi =
          registration[i].Registration_course[j].note >= 14
            ? aprovedPromedi + registration[i].Registration_course[j].note
            : aprovedPromedi;
        totalAprovedCredit =
          registration[i].Registration_course[j].note >= 14
            ? totalAprovedCredit +
        ...
        tempBody.push([
          ...
          typeCourse,
          registration[i].Registration_course[j].credits,
          registration[i].Registration_course[j].note + " " + noteText,
        ]);
      }
      pdf.setFont("helvetica", "bold");
      pdf.text("SEMESTRE", 10, initialTableY - 2);
      ...
      pdf.text(
        registration[i].Academic_semester.Academic_calendar.denomination +
          " - " +
          academicSemester,
        37,
        initialTableY - 2
...
const pdfReportAutoTableActa = async (record) => {
  ...
  pdf.autoTable({
    ...
    startY: yAxis + 35,
    head: [["#", "APELLIDOS Y NOMBRES", "CODIGO", "NOTA LETRA", "NOTA NÚMERO"]],
    body: data,
  });
  ...
  data.map((r, index) => {
    ...
    if (r.Registration.length > 0) {
      r.Registration.map((m, i) => {
        algo =
          m.Academic_semester.Academic_calendar.denomination.substr(-4) +
          " - " +
