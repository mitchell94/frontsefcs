    ...
    render() {
        ...
        const options = {
            ...
            onDownload: (buildHead, buildBody, columns, data) => {
                ...
                component.pdfReportAutoTable("ESCUELA DE SE UNSM", "REPORTE DE PROGRAMAS DE ESTUDIO", columns, data);
                return false;
            },
        ...
        const columns = [
            ...
            },
            {
                ...
                options: {
                    filter: true,
                    sort: true,
                    customFilterListOptions: {render: v => `UNI. ORG ADMINISTRADORA: ${v}`},
                    customBodyRender: (value, tableMeta, updateValue) => {
...

            {
                ...
                options: {
                    filter: true,
                    sort: true,
                    customFilterListOptions: {render: v => `FACULTAD: ${v}`},
                    customBodyRender: (value, tableMeta, updateValue) => {
                    ...
                    customBodyRender: (value, tableMeta, updateValue) => {
                        ...
                        if (value === undefined) {
                            ...
                            if (value === "") {
                                ...
                                return value ? <span className="badge badge-primary inline-block">Habilitado</span> :
                ...
                options: {
                    ...
                    customBodyRender: (value, tableMeta, updateValue) => {
                        ...
                        if (value === undefined) {
                            ...
                            return value ?
                                <>
                                    <OverlayTrigger overlay={<Tooltip>PLANES DE ESTUDIO</Tooltip>}>
                                        <Link to={"/programs/" + btoa(value.id) + "/study-plan"}> <Book className={"text-warning"}/></Link>


                                    </OverlayTrigger>
                                    <OverlayTrigger overlay={<Tooltip>PLANES DE ADMISIÓN</Tooltip>}>
                                        <Link to={"/programs/" + btoa(value.id) + "/admission-plan"}> <Assessment className={"text-purple"}/></Link>
                                    ...
                                    {/*</OverlayTrigger>*/}
                                    <OverlayTrigger
                                        overlay={<Tooltip style={{zIndex: 100000000}}>DOCUMENTOS</Tooltip>}>
                                        <Link to={"/programs/" + btoa(value.id) + "/document"}> <Description className={"text-secondary"}/></Link>

                                    </OverlayTrigger>
                                    <OverlayTrigger
                                        overlay={<Tooltip>EDITAR</Tooltip>}>
                                        <Edit style={{color: "#1d86e0"}} onClick={() => this.props.retriveData(value)}/>
