...
export default class Amortization extends Component {
    ...
    componentDidMount() {
        ...
        this.getPaymentStudentSemester(this.props.studentID);
        this.setState({
    ...
    */
    getPaymentStudent(studentID) {
        ...
        axios.get(url, app.headers).then(res => {
            if (res.data) this.setState({monthlys: res.data, imgSize: "wid-100",});
    ...
    /*
    * B-CONCEPTOS-ESTUDIANTE
    *  listPaymentStudentSemester
    * FUNCION PARA CARGAR LOS  EL CRONOGRAMA DE PAGOS DEL ESTUDIANTE con semestres establecidos
    */
    getPaymentStudentSemester(studentID) {
        const url = app.accounting + '/' + app.payment + '/list/semester/' + studentID;
        axios.get(url, app.headers).then(res => {
            if (res.data) this.setState({semesterPayment: res.data, imgSize: "wid-100",});
    ...
    pendingVoucherGenerate(type) {
        this.setState({disabled: true});

        const url = app.accounting + '/' + app.payment + '/pending/generate';
        let concepts;
        if (type) {
            const {semesterPayment} = this.state;
            concepts = this.selectedConceptNullSemester(semesterPayment);
        } else {
            const {monthlys} = this.state;
            concepts = this.selectedConceptNull(monthlys);
        }
        ...
        if (concepts.length > 0) {
            ...
            axios.patch(url, data, app.headers).then(() => {
                ...
                this.getPaymentStudentSemester(this.state.studentID);
   ...
   */
    ...
    */
    ...
    selectedConceptPayment = (semester, payment) => {

        let total = this.state.totalInversion;
        let data = [];
        data = this.state.semesterPayment;
        for (let n = 0; n < data.length; n++) {
            if (data[n].id === semester) {
                for (let i = 0; i < data[n].Payment.length; i++) {
                    if (data[n].Payment[i].id === payment.id && data[n].Payment[i].order_number === payment.order_number) {
                        data[n].Payment[i].state = !payment.state;
                        // data[i].exists = !k.exists;
                        if (data[n].Payment[i].state) {
                            total = total + parseFloat(data[n].Payment[i].amount);
                        } else {
                            total = total - parseFloat(data[n].Payment[i].amount);
                        }
                    }
        ...
        this.setState({totalInversion: total, semesterPayment: data})
    };

    selectedConceptNullSemester = (record) => {
        //retorna solo los conceptos selecccionados con estado true

        let data = record;
        let tempArray = [];
        for (let n = 0; n < data.length; n++) {
            for (let i = 0; i < data[n].Payment.length; i++) {
                if (data[n].Payment[i].state && data[n].Payment[i].type === "Nulo") {
                    tempArray.push(data[n].Payment[i]);
                }
            }
        }
    ...
    render() {
        ...
        const { monthlyPendings, monthlyRealizeds,  semesterPayment} = this.state;
        ...
        return (
            ...
            <>
                ...
                <Row>
                    ...
                    <Col xl={12}>
                        {
                            semesterPayment.length > 0 && semesterPayment.map((j, i) => {
                                return (
                                    <Card style={{marginBottom: "14px"}} key={i}>
                                        ...
                                        <Card.Header className='h-40' style={{height: '40px', marginBottom: '-1px'}}>
                                            ...
                                            <div className="d-flex align-items-center justify-content-between " style={{marginTop: '-15px'}}>
                                                <div className="d-inline-block">
                                                    <h5>{j.AS.denomination + " - " + j.AS.AC.denomination.replace('CALENDARIO ACADEMICO', '')} </h5>
                                                </div>
                                                {
                                                    i === 0 &&
                                                    <div className="text-right">
                                                        {/*<p className="m-0 font-weight-600"><span className="text-primary">84%</span></p>*/}
                                                        <button type="button" onClick={() => this.pendingVoucherGenerate(true)}
                                                                className="btn btn-block btn-sm btn-danger mt-md-0 mt-2">
                                        ...
                                        <Card.Body className='card-task pb-0 pt-0 pl-0 pr-0'>
                                            ...
                                            <Table size="sm" hover responsive style={{width: '100%'}}>
                                                ...
                                                <tbody>
                                                {j.Payment.length > 0 && j.Payment.map((r, i) => {
                                                                ...
                                                                <div className="d-inline-block align-middle">
                                                                    <label style={{marginBottom: '-6px'}}
                                                                        ...
                                                                        <input type="checkbox" className="custom-control-input"
                                                                               onClick={() => this.selectedConceptPayment(j.id, r)}
                                                                               readOnly
                    ...
                    {/*            <button type="button" onClick={() => this.pendingVoucherGenerate(false)}*/}
