...
class DataTable extends Component {
  ...
  async listRegistrationCourseByRegistration(id_registration) {
    ...
    const url =
      app.registration + "/registration-course/registration/" + id_registration;
    try {
    ...
    const options = {
      ...
      customToolbar: () => {
        if (optionEdit) {
          return (
            <>
              <OverlayTrigger overlay={<Tooltip>DESCARGAR FICHA</Tooltip>}>
                <button
              ...
              {this.props.activate !== 0 && (
                <>
                  ...
                  >
                    <button
                      onClick={() => this.openModal(records.id)}
                      type="button"
                      style={{
                        backgroundColor: "white",
                        borderColor: "white",
                      ...
                      className="btn-icon btn btn-dark"
                    >
                      <Edit style={{ color: "#bf009c" }} />
                    </button>
                  </OverlayTrigger>
                  <OverlayTrigger overlay={<Tooltip>Eliminar</Tooltip>}>
                    <button
                      onClick={() =>
                        this.props.deleteSweetRegistration(records.id)
                      }
                      type="button"
                      style={{
                        backgroundColor: "white",
        ...
        options: {
          ...
          customBodyRender: (value, tableMeta, updateValue) => {
            if (value === undefined) {
              ...
              return value ? (
                <span className={"badge  inline-block " + state}>{value}</span>
              ) : (
                "No def."
              );
            }
            ...
            <Row>
              <Col xs={12} sm={12} md={12} lg={12} xl={12}>
                <Alert variant={"success"}>
                  Cuado el estudiante solicita retirarse de los demas cursos una
                  vez iniciado el semestre. <br /> Esto para que no quede
              ...
              <Table size="sm" hover style={{ width: "100%" }}>
                <tbody>
                  {this.state.registrationCurses.length > 0 &&
                    this.state.registrationCurses.map((r, j) => {
                      return (
                        <tr
                          className="d-flex"
                          key={j}
                        ...
                        >
                          ...
                          <td className="col-4" style={{ paddingTop: "0.1em" }}>
                            <Form.Group className="form-group fill">
                              ...
                              >
                                <option value="Aprobado" hidden>
                                  Aprobado
                                </option>
                                <option value="Desaprobado" hidden>
                                  Desaprobado
                                </option>
