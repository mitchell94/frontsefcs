...
class UnitMeasure extends Component {
    ...
    async save() {
        ...
        const {denomination, equivalence} = this.state;


        if (denomination !== '' && equivalence !== '') {
            const url = app.general + '/' + app.unitMeasure;
            let data = new FormData();
            data.set('description', denomination);
            data.set('equivalence', equivalence);
    ...
    async update() {
        ...
        if (denomination !== '' && equivalence !== '') {
            ...
            const url = app.general + '/' + app.unitMeasure + '/' + this.state.actualID;
            let data = new FormData();
            data.set('description', denomination);
            data.set('equivalence', equivalence);
    ...
    async delete(id) {
        try {
            const url = app.general + '/' + app.unitMeasure + '/' + id;
    ...
    handleChange = field => event => {
        switch (field) {
            case 'denomination':
                ...
                break;
            case 'equivalence':
                this.setState({equivalence: event.target.value});
                break;
    ...
    render() {
        ...
        return (
            <>
                ...
                <TitleModule
                    actualTitle={"UNIDADES DE MEDIDA"}
                    actualModule={"UNIDADES DE MEDIDA"}
                    ...
                    <Modal.Body>
                        <Row>
                            ...
                            <Col xs={12} sm={12} md={12} lg={12} xl={12}>
                                <Form.Group className="form-group fill">
                                    <Form.Label className="floating-label">Equivalencia</Form.Label>
                                    <Form.Control
                                        type="number"
                                        value={equivalence}
                                        name={"equivalence"}
                                        onKeyPress={this.handleKeyPress}
                                        onChange={this.handleChange('equivalence')}
