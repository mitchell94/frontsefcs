...
import DataTablePaymentRegistrate from "./DataTablePaymentRegistrate";
...
class Payment extends Component {
    ...
    async componentDidMount() {
        ...
        if (component.ORGANIC_UNIT !== "") {
            ...
            this.listMovementPendientByOrganicUnit(component.ORGANIC_UNIT)
        }
        ...
        try {
            ...
            if (res.data) {
                ...
                res.data.map(r => {
                    ...
                    let _orderNumber = r.order_number ? r.order_number : '';
                    temp.push({
                        "id": r.id,
                        "payment_date": r.payment_date,
                        "concept": r.Concept.denomination + ' ' + _orderNumber,
                        "amount": r.amount,
        ...
        try {
            ...
            if (res.data) {
                this.setState({
                    ...
                    totalBalance: (res.data.totalMovement - res.data.totalConcept).toFixed(1)
                })
            }
            this.setState({movementLoader: false});
        } catch (err) {
    ...
    async listMovementPendientByOrganicUnit(id_organic_unit) {
        // this.setState({movementLoader: true});
        const url = app.accounting + '/' + app.movement + '/pendient/' + id_organic_unit;
        try {
    ...
    handleChange = field => event => {
        switch (field) {
            ...
            case 'organicUnit':
                ...
                this.listMovementPendientByOrganicUnit(event.target.value)
                break;
    ...
    render() {
        ...
        const {persons, person, paymentRegistrateLoader} = this.state;
        ...
        return (<>
                ...
                <TitleModule
                    actualTitle={"INGRESOS"}
                    actualModule={"INGRESOS"}
                                ...
                                {this.state.searchPerson ? <Form.Group className="form-group fill">
                                        ...
                                        <Table hover responsive style={{marginTop: '-1px'}}>
                                            ...
                                            {persons.length > 0 && persons.map((r, i) => {
                                                ...
                                                let spanP = r.Student.type === 'Retirado' ? 'badge-warning' : r.Student.type === 'Abandonado' ? 'badge-danger' : r.Student.type === 'Postulante' ? 'badge-info' : r.Student.type === 'Estudiante' ? 'badge-primary' : 'badge-success';
                ...
                {this.state.searchPerson === false ? <Row>
                    ...
                    <div style={{position: 'relative'}}>
                        {this.state.paymentRegistrateLoader && component.spiner}
                        <DataTablePaymentRegistrate

                            records={this.state.movementPendients}
