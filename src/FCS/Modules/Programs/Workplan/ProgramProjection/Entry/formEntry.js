...
class FormEntry extends React.Component {
    ...
    async createEntry(workPlanID) {
        this.setState({loaderEntry: true});

        const {concept, cant, amount, observation} = this.state;

        if (workPlanID !== '' && concept.value !== '' && cant !== '' && amount !== '') {
            const url = app.programs + '/' + app.entryWorkPlan;
            ...
            try {
                ...
                this.setState({concept: "", cant: "", amount: "", observation: "", subTotal: ""});
                this.props.callData();
    ...
    async updateEntry() {
        ...
        if (concept !== '' && cant !== '' && amount !== '') {
            ...
            const url = app.programs + '/' + app.entryWorkPlan + '/' + this.state.actualEntryID;
            let data = new FormData();
            data.set('id_concepts', concept.value);
            data.set('cant', cant);
            data.set('amount', amount);
    ...
    render() {
        ...
        const {concepts, concept, cant, amount, subTotal, observation} = this.state;
        const {formEntry, action, loaderEntry} = this.state;
                            ...
                            <Col xs={12} sm={12} md={12} lg={12} xl={12}>
                                ...
                                {action === 'add' ?
                                    <Button
                                        ...
                                        disabled={loaderEntry}
                                        variant="primary"

                                        onClick={() => this.createEntry(workPlanID)}>
                                        {loaderEntry && <span className="spinner-border spinner-border-sm mr-1" role="status"/>}
                                        Guardar</Button> :
                                    <Button
                                        className="pull-right"
                                        disabled={loaderEntry}
