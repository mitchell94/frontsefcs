...
export default class ContractTeacher extends Component {
    ...
    getContractsByUser() {
        const url = app.security + '/' + app.teacher + '/' + this.props.userID;
        axios.get(url, app.headers).then(res => {
            if (res.data) {
    ...
    getRoles() {
        const url = app.security + '/' + app.role + '/type/Docente';
        axios.get(url, app.headers).then(res => {
            if (res.data) {
    ...
    saveContractTeacher = () => {
        ...
        const url = app.security + '/' + app.teacher;
    ...
    updateContractTeacher = () => {
        this.setState({disabled: true})
        const url = app.security + '/' + app.teacher + '/' + this.state.currentID;
        const {userID, id_organic_unit, rolID, chargeID, contractTypeID, date_start, date_end} = this.state;
        if (userID !== '' && id_organic_unit !== '' && rolID !== '' && chargeID !== '' && contractTypeID && date_start !== '') {
            let data = new FormData();
            data.set('id_user', userID);
    ...
    handleOpenSweetAlertWarning = (id) => {
        Swal.fire({
            ...
            if (result.value) {
                this.deleteContractTeacher(id);
            }
        })
    };

    deleteContractTeacher = id => {
        const url = app.security + '/' + app.teacher + '/' + id;
        ...
        return (
            <Card>
                ...
                <Card.Body className={isContractAction === 'register' ? 'border-top pro-det-edit collapse show' : 'border-top pro-det-edit collapse'}>
                    ...
                    <Row className='form-group'>
                        <label className="col-sm-3 col-form-label font-weight-bolder">Rol</label>
                        <Col sm={9}>
                            <Form.Control as="select"
                                          value={rolID}
                    ...
                    {action === 'add' ?
                        <Button
                            ...
                            disabled={disabled}
                            variant="primary"
                            onClick={() => this.saveContractTeacher()}>
                        ...
                        <Button
                            ...
                            disabled={disabled}
                            variant="primary"
                            onClick={() => this.updateContractTeacher()}>
                            {disabled && <span className="spinner-border spinner-border-sm mr-1" role="status"/>}
