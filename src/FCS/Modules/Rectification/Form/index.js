...
class FormAcademicRecord extends React.Component {
    ...
    componentDidUpdate(prevProps, prevState) {
        if (prevProps.formRegistration !== this.props.formRegistration && prevProps.action !== this.props.action) {
            ...
            this.props.formRegistration && this.props.action === "add" && this.listRegistrationCourse(this.props.studyPlanID, this.props.studentID)
    ...
    async listConceptByID(description, id) {
        ...
        try {
            const res = await axios.get(url, app.headers);
            if (res.data) {
                let recargo = description === 'Extemporánea' ? Math.round(res.data.percent / 100 * this.state.amountUit * 0.1) : 0;
                if (description === 'Otros' || description === 'Tesis') {
    ...
    async createRegistration() {
        ...
        const {studentID, admissionPlanID} = this.props;
        let tempCourse = [];
        ...
        if (tempCourse.length > 1 && this.state.multipleCredit) {
            ...
            if (tempCourse.length > 0 && concept !== '' && amount !== '' && process !== '' && typeRegistration !== '' && studentID !== '' && admissionPlanID !== '') {
        ...
        switch (field) {
            ...
            case 'typeRegistration':
                if (event.target.value === 'Extemporánea') {
                    ...
                    //maestria 65 //doctorado 63
                    let id = this.state.academicDegree === 'Doctor' ? 63 : 65;
                    this.listConceptByID('Otros', id)


                } else if (event.target.value === 'Curso dirigido Tesis') {
                    //maestria 64  //doctorado 62
                    let id = this.state.academicDegree === 'Doctor' ? 62 : 64;
